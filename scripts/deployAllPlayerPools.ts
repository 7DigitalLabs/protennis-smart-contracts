...
/**
 * Script to deploy PlayerSharePool for all players in the MongoDB database
 * and update each player's `contractAddress` field.
 *
 * Deploys a MockUSDC first and mints to the admin.
 *
 * Usage:
 *   yarn hardhat run scripts/deployAllPlayerPools.ts --network baseSepolia
 *
 * Required env vars (.env):
 *   MONGODB_URI              - MongoDB connection URI
 *   ADMIN_ADDRESS            - Contract admin (also receives MockUSDC)
 *   ORDER_SIGNER_ADDRESS     - Signer for EIP-712 orders
 *   CSP_TREASURY_ADDRESS     - Treasury that receives fees
 *   CSP_TARGET_RAISE         - Target raise in USDC (e.g.: "7500")
 *   CSP_PRICES               - Prices per bin (e.g.: "0.0075,0.0100,0.0125")
 *   CSP_SHARE_COUNTS         - Share quantities per bin (e.g.: "16162,32323,48485")
 *
 * Optional:
 *   REGISTRY_ADDRESS         - If set, registers each pool on the registry
 *   SKIP_VERIFICATION        - If "true", skips explorer verification
 *   BATCH_SIZE               - Number of players to process per batch (default: 10)
 *   DRY_RUN                  - If "true", simulates without actual deployment
 *   MOCK_USDC_MINT_AMOUNT    - Amount of MockUSDC to mint to admin (default: 1000000000 = 1B)
 */
...
requiredEnv("MOCK_USDC_ADDRESS", MOCK_USDC_ADDRESS);
...
// --- Main ---
async function main() {
    console.log("Starting batch deployment of PlayerSharePools...\n");
    ...
    // Connect to network first to deploy MockUSDC
    const { viem } = await network.connect();
    
    let mockUsdcAddress: `0x${string}`;
    
    if (DRY_RUN) {
        mockUsdcAddress = "0x_DRY_RUN_MOCK_USDC" as `0x${string}`;
        console.log(`   [DRY_RUN] Would deploy MockUSDC`);
    } else {
        mockUsdcAddress = MOCK_USDC_ADDRESS!;
        // Mint MockUSDC to admin

        const mockUsdc = await viem.getContractAt("MockUSDC", mockUsdcAddress);
        // Check balance
        const balance = await mockUsdc.read.balanceOf([ADMIN_ADDRESS!]);
        console.log(`   Admin balance: ${Number(balance) / 1e6} USDC\n`);
    }
    ...
    for (let i = 0; i < players.length; i += BATCH_SIZE) {
        ...
        for (const player of batch) {
            ...
            try {
                // Deploy PlayerSharePool with MockUSDC
